...
var App = function () {
    ...
    var handleMyScript = function() {
    	//针对pageNav.ftl的href组件
        ...
        $('#saveNewPassword').click(function(e) {
        	...
        	AdminBean.changePassword(oldpassword, newpassword, function(){
    				alert("保存成功!");
    				location.reload();
        		});
        });
        ...
        if(typeof AdminBean != "undefined"){
	        AdminBean.getServerMap(function(result){
	        	var serverMap = $.parseJSON(result);
	        	var oprNameMap = {};
        		for(var sm in serverMap){
        			oprNameMap[sm] = serverMap[sm][0].oprGroupName;
	        	}
	        	function changChild(oprGroup){ 
	        		var _server = serverMap[oprGroup];
	        		$("#serverIdSel").html("");
	        		$("#serverIdSel").append("<option value='0'>ALL</option>");
	        		if(_server){
		    	        for(var i=0;i<_server.length;i++){  
		    	            $("#serverIdSel").append("<option value='"+_server[i].serverId+"'>"+_server[i].serverId+"</option>");  
		    	        }  
	        		}
	        		
	        	}  
	
	        	$("#oprGroupSel").append("<option value=''>ALL</option>");
	        	for(var sm in serverMap){
	        		$("#oprGroupSel").append("<option value='"+sm+"'>"+oprNameMap[sm]+"</option>");
	        	}
	        	$("#oprGroupSel").change(function(){  
	                changChild($(this).val());  
	            });
	        	
	        	//初始化当前值
	        	$("#oprGroupSel").val($("#oprGroupSel").attr("sval"));
	        	changChild($("#oprGroupSel").attr("sval"));
	        	$("#serverIdSel").val($("#serverIdSel").attr("sval"));
			});
        }
    }
