									...
									<table
										...
										<thead>
											<tr class="text-c" role="row">
												<th><@spring.message "gms.tool.buildlist.itemname"/></th>
												<th><@spring.message "gms.tool.buildlist.templateid"/></th>
												<th><@spring.message "gms.tool.buildlist.createTime"/></th>
												<th><@spring.message "gms.tool.buildlist.order"/></th>
											</tr>
										</thead>
										...
										<#list datas as dt>
							                <tr class="text-c">
							                	<td>${dt.buildname?default("")}</td>
						                        <td>${dt.templateId?default("")}</td>
                                                 <td>${FsGameGmsUtils.formatDate(dt.createTime!0, "yyyy-MM-dd HH:mm:ss")}</td>
							                	<td class="td-manage">
							                	<a style="text-decoration:none" class="btn btn-danger radius" name="" onClick="return confirm('确定删除?');" href="?module=playeritem&action=playbuild&oprGroup=${oprGroup}&serverId=${serverId}&accountName=${accountName}&id=${id}&dbID=${dbID}&buildid=${dt.id}"><@spring.message "gms.tool.builddel"/></a>
							                	</td>
							            	</tr>
							            </#list>
										</tbody>
