									...
									<table
										...
										<thead>
											<tr class="text-c" role="row">
												<th><@spring.message "gms.tool.playmessage.id"/></th>
												<th><@spring.message "gms.tool.playmessage.content"/></th>
												<th><@spring.message "gms.tool.playmessage.receive"/></th>
												<th><@spring.message "gms.tool.playmessage.receivetime"/></th>
												<th><@spring.message "gms.tool.playmessage.tool"/></th>
											</tr>
										</thead>
										...
										<#list datas as dt>
							                <tr class="text-c">
							                	<td>${dt.dbId?default("")}</td>
						                        <td>${dt.content?default("")}</td>
                                                 <td>${dt.receiverId?default("")}</td>
                                                 <td>${FsGameGmsUtils.formatDate(dt.createTime!0, "yyyy-MM-dd HH:mm:ss")}</td>
							                	<td class="td-manage">
							                	<a style="text-decoration:none" class="btn btn-danger radius" name="" onClick="return confirm('确定删除?');" href="?module=playeritem&action=playmessage&oprGroup=${oprGroup}&serverId=${serverId}&accountName=${accountName}&id=${id}&dbID=${dbID}&messageid=${dt.dbId}"><@spring.message "gms.tool.playmessage.del"/></a>
							                	</td>
							            	</tr>
							            </#list>
										</tbody>
